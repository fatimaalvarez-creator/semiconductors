...
- `uart_top_tb.v` → Prueba del sistema completo
  ...
  En la gráfica de simulación se observa el comportamiento correcto del sistema UART en modo transmisión, gestionado desde el módulo `uart_top_tb`. Al inicio, el sistema se mantiene en estado de reposo (`send_data = 0`) y con `rst` desactivado, permitiendo que la lógica FSM permanezca estable y sincronizada al reloj (`clk`).
